...
def ocr_table_cells(img, cells, table_rows_cols, ocr_model):
    ...
    excel_font_size = 11 # in pixels, in point (pt) is 14
...
def convert_cells_to_luckysheet(cells, row_heights, column_widths, luckysheet_font_size = 11):
    ...
    for idx, row in enumerate(row_heights):
        row_height_data[idx] =row
    for idx, col in enumerate(column_widths):
        column_width_data[idx] = col
    ...
    luckysheet = {'celldata': cell_data, 'merge': merge_data, "rowlen": row_height_data, 'columnlen': column_width_data, 'row': len(row_heights), 'column': len(column_widths)}
